...
function generateMeta(context: TransformContext, hostname: string) {
    ...
    if (pageData.frontmatter.type)
        head.push(['meta', {name: 'og:type', content: pageData.frontmatter.type}]);
    ...
    if (pageData.frontmatter.image) {
        head.push(['meta', {property: 'og:image', content: `${hostname}/${pageData.relativePath.replace(/^\//, '')}`}]);
        head.push(['meta', {name: 'twitter:image', content: `${hostname}/${pageData.relativePath.replace(/^\//, '')}`}]);
    } else {
